...
pca_res_log <- prcomp(log_counts, scale. = TRUE)

# Extraer las coordenadas de los primeros dos componentes principales (PC1 y PC2)
pca_df <- as.data.frame(pca_res_log$x)  # Esto contiene las puntuaciones de los componentes principales
pca_df$Muscle.loss <- colData(se_object)$`Muscle loss`  # Añadir la columna de metadatos (Muscle.loss)

# Extraer las coordenadas de los metabolitos
pca_var <- as.data.frame(pca_res_log$rotation[, 1:2])  
pca_var$Metabolito <- rownames(pca_var)  # Nombres de metabolitos


## Determinar el porcentaje de variación explicado por cada PC.
std_devs <- pca_res_log$sdev
